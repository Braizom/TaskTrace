      ...
      <li v-for="(list, i) in theme.lists" :key="i">
        <details class="group">
          <summary class="w-full flex justify-between gap-3 px-4 py-3 font-medium marker:content-none hover:cursor-pointer">
            ...
            <button type="button" @click="deleteList(i)" class="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 text-center inline-flex items-center">
              Delete ToDo List
          ...
          <ToDoListElem :list="list" :listId="i" :createElementTest="createElementTest" :deleteElement="deleteElement"/>
        </details>
      </li>
    </ul>
  </div>
